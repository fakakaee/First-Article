function [Model,Y] = WELM(TrainingData_File)

C=2^1;
NumberofHiddenNeurons = 200;
ActivationFunction='radbas';   %Type of activation function:
...
%         result(k,1)=TestingAccuracy;
%         
%         test_sensitivity = TP/(TP+FN);
%         result(k,2)=test_sensitivity;
%         
%         test_specificity = TN/(TN+FP);
%         result(k,3)=test_specificity;
%         
%         test_gmean = sqrt(test_sensitivity * test_specificity);
%         result(k,4)=test_gmean;
%         
%     end
% end
% 
...
% disp( mean(result(:,1)));
% disp('test_sensitivity=');
% disp( mean(result(:,2)));
% disp('test_specificity=');
% disp( mean(result(:,3)));
% disp('test_gmean=');
% disp( mean(result(:,4)));
